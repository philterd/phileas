...
public class CityFilterTest extends AbstractFilterTest {
    ...
    public void filterCitiesClose() throws IOException {
        ...
        final LuceneDictionaryFilter filter = new LuceneDictionaryFilter(FilterType.LOCATION_CITY, strategies, INDEX_DIRECTORY, LuceneDictionaryFilter.getCitiesDistances().get(SensitivityLevel.LOW), anonymizationService);
        filter.close();
    ...
    public void filterCitiesExactMatch() throws IOException {
        ...
        final List<CityFilterStrategy> strategies = Arrays.asList(new CityFilterStrategy());
        final LuceneDictionaryFilter filter = new LuceneDictionaryFilter(FilterType.LOCATION_CITY, strategies, INDEX_DIRECTORY, LuceneDictionaryFilter.getCitiesDistances().get(SensitivityLevel.MEDIUM), anonymizationService);
    ...
    public void filterCitiesExactMatch2() throws IOException {
        ...
        final List<CityFilterStrategy> strategies = Arrays.asList(new CityFilterStrategy());
        final LuceneDictionaryFilter filter = new LuceneDictionaryFilter(FilterType.LOCATION_CITY, strategies, INDEX_DIRECTORY, LuceneDictionaryFilter.getCitiesDistances().get(SensitivityLevel.HIGH), anonymizationService);

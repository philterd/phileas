...
public class LocationsAnonymizationService extends AbstractAnonymizationService {
    ...
    private static final Logger LOGGER = LogManager.getLogger(LocationsAnonymizationService.class);

    private DataGenerator dataGenerator;

    public LocationsAnonymizationService(final ContextService contextService, final Random random, final AnonymizationMethod anonymizationMethod) {
    ...
    public LocationsAnonymizationService(final ContextService contextService, final Random random, final List<String> candidates) {
    ...
    public LocationsAnonymizationService(final ContextService contextService, final Random random) {
    ...
    public LocationsAnonymizationService(final ContextService contextService) {
    ...
    public String anonymize(final String token) {
        ...
        if (anonymizationMethod == AnonymizationMethod.FROM_LIST) {
            ...
            // REALISTIC_REPLACE
            String anonymized = dataGenerator.cities().random();

            while (anonymized.equalsIgnoreCase(token)) {
                anonymized = dataGenerator.cities().random();
            }

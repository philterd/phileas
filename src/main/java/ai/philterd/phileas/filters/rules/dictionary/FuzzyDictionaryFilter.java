...
public class FuzzyDictionaryFilter extends DictionaryFilter {
    ...
    @Override
    public FilterResult filter(Policy policy, final String context, int piece, String input, Map<String, String> attributes) throws Exception {
        ...
        if(policy.getIdentifiers().hasFilter(filterType)) {
            ...
            for(final String entry : dictionary.keySet()) {
                ...
                if (matcher.find()) {
                    ...
                    spans.add(createSpan(input, startPosition, startPosition + entry.length(), 1.0, context, entry, policy, attributes));
                            ...
                            if (ngram.length() > 2) {
                                ...
                                if (requireCapitalization && Character.isUpperCase(ngram.charAt(0))) {
                                    ...
                                    if (sensitivityLevel == SensitivityLevel.HIGH && distance < 1) {
                                        spans.add(createSpan(input, start, end, 0.9, context, entry, policy, attributes));
                                    } else if (sensitivityLevel == SensitivityLevel.MEDIUM && distance <= 2) {
                                        spans.add(createSpan(input, start, end, 0.7, context, entry, policy, attributes));
                                    } else if (sensitivityLevel == SensitivityLevel.LOW && distance < 3) {
                                        spans.add(createSpan(input, start, end, 0.5, context, entry, policy, attributes));
                                    }
    ...
    private Span createSpan(String text, int characterStart, int characterEnd, double confidence, String context,
                            String token, Policy policy, Map<String, String> attributes) throws Exception {

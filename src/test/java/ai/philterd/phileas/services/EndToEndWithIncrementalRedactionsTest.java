...
public class EndToEndWithIncrementalRedactionsTest {
    ...
    public void endToEndWithRedactionIncrements() throws Exception {
        ...
        final TextFilterResult response = service.filter(policy, "context", "George Washington whose SSN was 123-45-6789 was the first president of the United States and he lived at 90210.", MimeType.TEXT_PLAIN);
    ...
    public void endToEndWithoutRedactionIncrements() throws Exception {
        ...
        final PhileasFilterService service = new PhileasFilterService(phileasConfiguration, contextService, vectorService);
        final TextFilterResult response = service.filter(policy, "context", "George Washington was president.", MimeType.TEXT_PLAIN);

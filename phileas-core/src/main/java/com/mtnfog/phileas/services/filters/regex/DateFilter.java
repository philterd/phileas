...
public class DateFilter extends RegexFilter {
    ...
    public DateFilter(List<? extends AbstractFilterStrategy> strategies, AnonymizationService anonymizationService, AlertService alertService, boolean onlyValidDates, SpanValidator spanValidator, Set<String> ignored, Crypto crypto, int windowSize) {
        ...
        for(final String delimiter : delimiters) {
            ...
            filterPatterns.add(new FilterPattern.FilterPatternBuilder(Pattern.compile("\\b\\d{4}" + delimiter + "\\d{2}" + delimiter + "\\d{2}"), 0.75).withFormat("YYYY-MM-dd".replaceAll("-", delimiter)).build());
            filterPatterns.add(new FilterPattern.FilterPatternBuilder(Pattern.compile("\\b\\d{2}" + delimiter + "\\d{2}" + delimiter + "\\d{4}"), 0.75).withFormat("MM-dd-YYYY".replaceAll("-", delimiter)).build());

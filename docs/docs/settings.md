...
| Setting                      | Description                                                                                                                                  | Allowed Values    | Default Value |
|------------------------------|----------------------------------------------------------------------------------------------------------------------------------------------|-------------------|---------------|
| `regex.timeout.ms`           | The maximum time in milliseconds a single regular-expression match attempt may run before it is aborted as a suspected catastrophic-backtracking (ReDoS) pattern. While filtering, applies to user-supplied identifier and section patterns, and exceeding the budget fails filtering for that input rather than skipping the pattern. The same budget bounds the probe every user-supplied pattern (identifier, section, and ignored) is given when the policy is loaded. A value of `0` or less disables the guard, and also skips the probe, since nothing would stop one that does not finish. | An integer value. | `1000`        |

...
| Setting                      | Description                                                                                                                                  | Allowed Values    | Default Value |
|------------------------------|----------------------------------------------------------------------------------------------------------------------------------------------|-------------------|---------------|
| `regex.timeout.ms`           | The maximum time in milliseconds a single regular-expression match attempt may run before it is aborted as a suspected catastrophic-backtracking (ReDoS) pattern. Applies to user-supplied identifier and section patterns. Exceeding the budget fails filtering for that input rather than skipping the pattern. A value of `0` or less disables the guard. | An integer value. | `1000`        |

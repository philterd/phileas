...
| `terms`                   | A list of terms to use instead of the bundled word list. When given, only these terms are matched. When omitted, the bundled list is used.                                        | None                                                     |
| `fuzzy`                     | Uses an exact, case-insensitive dictionary lookup when set to `false`. Options `sensitivity` and `capitialized` are not used when set to `true`.                                                                                           | `false`                                                  |

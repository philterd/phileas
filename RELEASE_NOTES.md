...
* The `ABBREVIATE` redaction strategy now works on the dictionary-based name filters (surname, first name, and physician name), reducing a name to its initials as documented. Previously it produced initials only on the Ph-Eye NER path and silently fell back to full redaction for these filters.

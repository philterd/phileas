...
* Policy-supplied regular expressions are now validated when a policy is loaded rather than when a document first reaches them. Every identifier pattern, section start and end pattern, and ignored pattern (both on a filter and at the top level of the policy) is compiled, and then probed against canary inputs under the `regex.timeout.ms` budget. A pattern that does not compile, exhausts the budget, or overflows the stack throws `InvalidPolicyPatternException`, naming the filter, the failure, and the pattern. Previously such a pattern was only exercised by a document that happened to produce a matching token, so the same policy could filter one document successfully and fail on the next. The checks run on the filter cache's miss path, once per distinct policy, so per-document filtering is unaffected. The probe is a detector rather than a proof, and the existing runtime handling still applies to a pattern that only misbehaves on real input. (issue #358)
* The SSN and TIN filter now recognizes an identifier written with a Unicode hyphen in place of the ASCII one, and one wrapped across a line break. The soft hyphen (U+00AD), the dashes U+2010 through U+2015 (including the non-breaking hyphen U+2011), the minus sign (U+2212), and the small and fullwidth hyphen-minus forms (U+FE58, U+FE63, U+FF0D) are accepted wherever an ASCII hyphen is, and the horizontal separator now covers a tab and a non-breaking space as well as a plain space. A line break is a separator only when a hyphen precedes it, so `078-05-` followed by `1120` on the next line is one identifier while three numbers on three lines are not. The input is not normalized, so spans still index into the original text and the redaction covers the whole identifier, line break included. Non-ASCII digits and a break inside a digit group are not detected. (issue #385)
...
* Added the `MAP_REPLACE` filter strategy, which replaces a detected value with a replacement from a lookup table supplied in the policy. The table is built from inline `mappings` and/or `mappingFiles` (local TSV files), with inline entries taking precedence; keys match case-insensitively by default, or exactly when `caseSensitive` is `true`. A value absent from the table is handled by an optional `generator` (a named entry in the policy's new top-level `generators` block that calls a local Ollama endpoint inside the deployment boundary), and otherwise by the strategy's `fallbackStrategy` (a terminal strategy such as `REDACT`, defaulting to `REDACT`). Each generated value is validated before use: a blank value, a value equal to the original (after case normalization), or a value that is found to reintroduce sensitive information when re-scanned against the policy's filters is rejected and falls back, as does a generator failure or timeout, so a detected value is never left in the clear. In `CONTEXT` scope a repeated value resolves consistently and the generator is not invoked again for it. Available to all filter types. This uses redaction policy schema 1.2.0. See the documentation for details.
* Added an `ein` filter that detects U.S. Employer Identification Numbers (the federal tax ID) in the canonical form `NN-NNNNNNN` (two digits, hyphen, seven digits) at word boundaries. Only the hyphenated form is detected; a bare nine-digit run is left to the SSN filter, and when both `ein` and `ssn` are enabled a value in the `NN-NNNNNNN` form resolves to EIN. An optional `onlyValidPrefixes` (default `false`) restricts matches to two-digit prefixes the IRS currently issues. A policy configures it with `identifiers.ein` and an `einFilterStrategies` array of the standard strategies. This uses redaction policy schema 1.2.0. See the documentation for details. (issue #325)
* The phone-number filter now honors an optional `region` property (a single ISO 3166-1 alpha-2 string or an array of them, defaulting to `US`) that sets the region(s) used to detect national-format numbers written without an international `+` country code. Each configured region is scanned and the results are merged and de-duplicated; `+`-prefixed numbers are detected regardless. A policy without `region` behaves exactly as before. This uses redaction policy schema 1.2.0. See the documentation for details. (issue #308)
* The currency filter now detects the `€`, `£`, `¥`, and `₹` symbols and the `USD`, `EUR`, `GBP`, `JPY`, `CAD`, `AUD`, and `INR` codes, written before or after the amount, in both `1,234.50` and `1.234,50` formats. Anonymization keeps the original currency instead of forcing `$`. (issue #328)
* The IPv4 address filter no longer reports a partial address. `255.255.255.255` is detected in full, and `256.1.1.1` produces no span. (issues #335, #336)
* The age filter now detects an age written with a separator after an `age` or `aged` keyword, such as `Age: 47` as it appears in a structured clinical or intake record. A colon, equals sign, or hyphen is accepted, with or without surrounding whitespace, or omitted entirely, so `Age: 47`, `Age:47`, `Age = 47`, `Age - 47`, and `Age 47` are all detected, as is the spelled-out form (`Age: thirty-five`). The whitespace around a separator may include a line break, so `Age:` on one line with its value on the next is detected. Words ending in "age" (for example `coverage: 47`) are not matched, and dates such as `2026-01-15` are still not detected as ages. (issue #332)
* A URL span no longer runs past the end of the URL into the following sentence. (issue #342)
* Trailing punctuation is no longer part of a URL span. Sentence punctuation and closing delimiters at the end of a URL (`See http://example.com/page.`, `(http://example.com/page)`, `"http://example.com/page."`, `http://example.com/path...`) are left in the document rather than redacted with the URL, while the same characters inside a path, query, or fragment are still matched. A trailing period at the end of the input is now excluded as well, which makes a URL with a path behave the same as one without. Because a regular expression cannot balance delimiters, a URL whose path legitimately ends in a closing delimiter, such as `https://en.wikipedia.org/wiki/Foo_(bar)`, is matched up to but not including it. (issue #349)
* Phileas now declares `log4j-core` alongside `log4j-api` so both Log4j artifacts resolve to the same version in a consuming project. Previously `log4j-core` arrived only transitively at an older version while `log4j-api` was managed to a newer one, and a project whose only dependency was `ai.philterd:phileas` failed at startup with a `NoSuchMethodError` from `ServiceLoaderUtil.loadServices`. (issue #350)
* A regular expression that fails while matching now fails the document instead of being skipped. A user-supplied identifier, section, or ignored pattern that exceeds the `regex.timeout.ms` budget or overflows the stack throws `RegexMatchFailedException`, naming the filter and the pattern, and no partially filtered text is returned. Previously a timed-out pattern was skipped and the document was returned as though it had been fully filtered, and a stack overflow escaped as a `StackOverflowError`. (issues #352, #357)
* Social Security numbers written with nothing between them, such as `123-45-6789123-45-6789`, are now redacted as one span covering the run. Previously neither number was detected, and a fragment straddling the two was matched instead, leaving real SSN digits in the output. An SSN or TIN match can no longer begin or end partway through a longer run of digits, so a long account number is not partly redacted as an SSN, and separated values (`123-45-6789 123-45-6789`) still produce one span each. Relatedly, two spans that abut are no longer treated as overlapping, so both are kept. (issue #343)
* The dictionary-backed filters (`firstName`, `surname`, `city`, `county`, `state`, and `hospital`) now accept a `terms` list in the policy, as `customDictionary` already did. When `terms` is given, the word list bundled in the jar is not used and only the supplied terms are matched, so a deployment can narrow name or place detection to its own roster, or extend it, without replacing the bundled resource. Terms work with both the exact and the `fuzzy` lookup. Omitting `terms`, or giving an empty list, keeps the bundled list, so existing policies are unaffected. In PhiSQL the same list is set with `OPTIONS ('terms' = ['Smith','Jones'])`, where the key is quoted because `TERMS` is a keyword. (issue #345)
* **Breaking:** the physician-name filter is removed. `PhysicianNameFilter`, `PhysicianNameFilterStrategy`, the `PhysicianName` policy class, the `Identifiers.physicianName` field, and `FilterType.PHYSICIAN_NAME` are gone, and `identifiers.physicianName` is no longer part of the redaction policy schema. It was implemented only in Java (as a pre/post-nominal heuristic, never NER), was absent from the Python and .NET ports, and had no conformance cases, so it was the one rules-based filter that could not be held to cross-language parity. A policy that still contains `physicianName` is rejected by the schema; Phileas itself ignores the unknown key and loads the policy, so a policy left unchanged will silently stop redacting physician names. Detect them with a Ph-Eye filter instead. (issue #339, PhiSQL RFC philterd/phisql#35)
...
* A filter strategy can now set a `color` for the redaction boxes drawn over the spans it redacts when a PDF is rendered, overriding the policy-wide `config.pdf.redactionColor` for those spans. Boxes are filled one at a time, so a single document can use a different color per filter. The color resolves to the strategy `color`, then `config.pdf.redactionColor`, then black; a value that is neither a known name nor a 6-digit hex string renders black. The named colors now cover `black`, `white`, `red`, `orange`, `yellow`, `green`, `blue`, and `gray`, and a hex value such as `#ff8800` is accepted, where previously only `white`, `black`, `red`, and `yellow` were recognized and any other value fell back to black. Setting a color has no effect on text redaction. This uses redaction policy schema 1.2.0. See the documentation for details. (issue #324, PhiSQL RFC philterd/phisql#31)

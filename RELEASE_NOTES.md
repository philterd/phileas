...
* Fixed span disambiguation so it works as intended: it now learns from unambiguous spans over time, uses correct cosine-similarity scoring with deterministic cold-start handling, hashes context tokens case-insensitively, and honors the configured stop words and hash algorithm.
* Made the in-memory disambiguation vector store thread-safe so concurrently processed documents no longer lose accumulated counts or discard a context's training data.
* Added `FileBasedVectorService`, a persistent disambiguation vector store that loads on startup and saves on demand (or on `close()`), so the "learns over time" behavior survives process restarts.
